...
#
# Run an ActiveDrops parameter sweep from the repository root. The script
# reads SWEEP_* variables from a sweep config file, generates case-specific
# parameter files with incrementing CaseNo, then runs each case through
# runSimulation.sh (sequentially by default, optionally in parallel).
#
# Usage:
#   bash runParameterSweep.sh [sweep_file] [--exec source.c] [--threads N] [--parallel N] [--dry-run]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --parallel)
      if [[ -z "${2:-}" ]]; then
        echo "ERROR: --parallel requires a positive integer value." >&2
        usage
        exit 1
      fi
      MAX_PARALLEL="$2"
...
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/activedrops-sweep.XXXXXX")"
...
echo "========================================="
echo "ActiveDrops - Parameter Sweep"
echo "========================================="
...
for param_file in "${PARAM_FILES[@]}"; do
  ...
  pe_value="$(get_param_value "Pe" "$param_file")"
  level_value="$(get_param_value "MAXlevel" "$param_file")"
  tmax_value="$(get_param_value "tmax" "$param_file")"

  echo "-----------------------------------------"
  echo "Launching Case ${case_no}"
  echo "Pe=${pe_value:-NA}, MAXlevel=${level_value:-NA}, tmax=${tmax_value:-NA}"
  echo "-----------------------------------------"
